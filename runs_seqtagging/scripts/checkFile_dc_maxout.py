...
# SAMPLE USAGE:
# python3 ~/Act_func/sequence_tagging/arg_min/runs/scripts/checkFile_dc_maxout.py /work/scratch/se55gyhe/Act_func/outputs/doc_classification/newsgroup-results/maxout/*__4-basic*acc1.csv ~/Act_func/docClassification/NG/scripts/maxout-4 
...
mytype=dir.split("-")[-1]
#print(mytype)
if mytype.endswith("/"): mytype=mytype[:-1]
...
for fn in os.listdir(dir):
  ...
  hs[(opt,l,d,filters,learning_rate,recurrent_init,emb_dim,ks,datasize,mytype)] = fn
  allFiles.add(fn)
...
for ifn,fn in enumerate(sys.argv[1:-1]):
  ...
  try:
   ...
   opt,l,d,ks,recurrent_init,learning_rate,filters,emb_dim,datasize,mytype = z[1:11]
   datasize = datasize.split("-")[0]
   mytype = mytype.split("-")[0] 
   filn = hs[(opt,l,d,filters,learning_rate,recurrent_init,emb_dim,ks,datasize,mytype)]
   if filn in allFiles:
  ...
  if done==False:
    ...
    fn = hs[(opt,l,d,filters,learning_rate,recurrent_init,emb_dim,ks,datasize,mytype)]
