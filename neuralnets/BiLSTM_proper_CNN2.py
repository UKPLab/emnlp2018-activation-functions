...
def readCharEmbeds(fn,header=False):
  v={}
  i=0
  for line in open(fn):
    if header and i==0:
    ...
    x = line.split()
    tok = x[0]
    vec = np.array([float(y) for y in x[1:]])
    v[tok] = vec
  return v
...
class BiLSTM:
    ...
    params = {'miniBatchSize': 32, 'dropout': [0.25, 0.25], 'classifier': 'Softmax', 'LSTM-Size': [100],
              ...
              'charLSTMSize': 25, 'clipvalue': 0, 'clipnorm': 1}  # Default params
    ...
    def trainModel(self):
        ...
        if self.params['optimizer'] in self.learning_rate_updates and self.epoch in self.learning_rate_updates[
        ...
        iterator = self.online_iterate_dataset(trainMatrix, self.labelKey) if self.params[
                                                                                  'miniBatchSize'] == 1 else self.batch_iterate_dataset(
            trainMatrix, self.labelKey)
    ...
    def buildModel(self):
        ...
        tokens = Sequential()
        tokens.add(Embedding(input_dim=embeddings.shape[0], output_dim=embeddings.shape[1], weights=[embeddings],
                             trainable=False, name='token_emd'))

        casing = Sequential()
        # casing.add(Embedding(input_dim=len(casing2Idx), output_dim=self.addFeatureDimensions, trainable=True))
        casing.add(Embedding(input_dim=caseMatrix.shape[0], output_dim=caseMatrix.shape[1], weights=[caseMatrix],
                             trainable=False, name='casing_emd'))

        mergeLayers = [tokens, casing]

        if self.additionalFeatures != None:
            for addFeature in self.additionalFeatures:
                maxAddFeatureValue = max([max(sentence[addFeature]) for sentence in
                                          self.dataset['trainMatrix'] + self.dataset['devMatrix'] + self.dataset[
                                              'testMatrix']])
                addFeatureEmd = Sequential()
                addFeatureEmd.add(
                    Embedding(input_dim=maxAddFeatureValue + 1, output_dim=self.params['addFeatureDimensions'],
                              trainable=True, name=addFeature + '_emd'))
                mergeLayers.append(addFeatureEmd)

        # :: Character Embeddings ::
        if params["randomChar"] not in [None,"None","none",False,"False","false"]:
          useRandomEmbeddings = True
        else:
          useRandomEmbeddings = False
        useFixedEmbeddings = not useRandomEmbeddings
        if not useRandomEmbeddings:
          char2embed = readCharEmbeds("/work/scratch/se55gyhe/cemb.txt")
        print("userandom",useRandomEmbeddings,params["charEmbeddings"]) 
 
        if params['charEmbeddings'] not in [None, "None", "none", False, "False", "false"]:
            #print("FOO"); sys.exit(1)
            print(self.dataset["mappings"]["characters"],len(self.dataset["mappings"]["characters"])); sys.exit(1)
            charset = self.dataset['mappings']['characters']
            charEmbeddingsSize = params['charEmbeddingsSize']
            maxCharLen = self.maxCharLen
            charEmbeddings = []
            #print(charset,"xx",useRandomEmbeddings); 
            #print("-->",char2embed,"##"); sys.exit(1)
            for cc in charset:
                limit = math.sqrt(3.0 / charEmbeddingsSize)
                rr = np.random.uniform(-limit, limit, charEmbeddingsSize)
                if useRandomEmbeddings:
                  vector = rr
                else:
                  if cc in char2embed: 
                    print("It's in %s"%cc,len(charset),len(char2embed)); sys.exit(1)
                  vector = char2embed.get(cc,rr) 
            ...
            chars = Sequential()
            print(charEmbeddings.shape)
            chars.add(TimeDistributed(Embedding(input_dim=charEmbeddings.shape[0], output_dim=charEmbeddings.shape[1],
                                                weights=[charEmbeddings], trainable=useFixedEmbeddings, mask_zero=True),
                                      input_shape=(None, maxCharLen), name='char_emd'))
            ...
            if params['charEmbeddings'].lower() == 'lstm':  # Use LSTM for char embeddings from Lample et al., 2016
                ...
                chars.add(TimeDistributed(Convolution1D(charFilterSize, charFilterLength, border_mode='same'),
                                          name="char_cnn"))
...
# model.add(Bidirectional(
        ...
        #         #          dropout_U=params['dropout'][1]), name="varLSTM_" + str(cnt)))
              ...
              model.add(Bidirectional(LSTM(units=params['LSTM-Size'][0], activation=act, recurrent_activation='hard_sigmoid', 
                                recurrent_initializer=params['init'], return_sequences=True, dropout=params['dropout'][0],
                                recurrent_dropout=params['dropout'][1])))
            elif act=="leakyrelu" and False:
              model.add(SimpleRNN(units=params['LSTM-Size'][0], activation="linear",
                                recurrent_initializer=params['init'], return_sequences=True, dropout=params['dropout'][0],
                                recurrent_dropout=params['dropout'][1]))
        ...
        if params['classifier'].lower() == 'softmax':
            model.add(TimeDistributed(Dense(len(self.dataset['mappings'][self.labelKey]), activation='softmax'),
                                      name='softmax_output'))
            lossFct = 'sparse_categorical_crossentropy'
        elif params['classifier'].lower() == 'crf':
            model.add(TimeDistributed(Dense(len(self.dataset['mappings'][self.labelKey])), name='hidden_layer'))
            crf = ChainCRF()
            model.add(crf)
            lossFct = crf.sparse_loss
        elif params['classifier'].lower() == 'tanh-crf':
            model.add(TimeDistributed(Dense(len(self.dataset['mappings'][self.labelKey]), activation='tanh'),
                                      name='hidden_layer'))
            crf = ChainCRF()
            model.add(crf)
            lossFct = crf.sparse_loss
    ...
    def evaluate(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            # this renames only the test results
            os.rename(self.my_outputName,self.my_outputName+str(epoch)+"__"+str(dev_score)+"__"+str(test_score))
            self.my_epoch = epoch
            self.my_dev_score = dev_score
            self.my_test_score = test_score
            ...
            if dev_score > max_dev_score:
                ...
                if self.modelSavePath != None:
                    savePath = self.modelSavePath.replace("[DevScore]", "%.4f" % dev_score).replace("[TestScore]",
                                                                                                    "%.4f" % test_score).replace(
                        "[Epoch]", str(epoch))
            ...
            if self.resultsOut != None:
                self.resultsOut.write(
                    "\t".join(map(str, [epoch + 1, dev_score, test_score, max_dev_score, max_test_score])))
                self.resultsOut.write("\n")
        ...
        self.params['scores'].append(max_test_score)
        self.params['dev_scores'].append(max_dev_score)

    def computeScores(self, devMatrix, testMatrix):
        return self.computeF1MacroScores(devMatrix, testMatrix)
    ...
    def computeF1MacroScores(self, devMatrix, testMatrix):
        dev_acc = self.computeF1Macro(devMatrix)
        test_acc = self.computeF1Macro(testMatrix)

        logging.info("Dev-Data: F1-Macro: %.4f" % (dev_acc))
        logging.info("Test-Data: F1-Macro: %.4f" % (test_acc))
    ...
    def computeF1(self, sentences, name=''):
        ...
        if self.writeOutput:
            self.writeOutputToFile(sentences, predLabels, '%.4f_%s' % (f1, name+self.flag))
        return pre, rec, f1

    def writeOutputToFile(self, sentences, predLabels, name):
        pp = self.modelSavePath.split("/")[-1]
        #savePath = pp.replace("[DevScore]", "%.4f" % self.my_dev_score).replace("[TestScore]",
        #                                                                                            "%.4f" % self.my_test_score).replace(
        #                "[Epoch]", str(self.my_epoch))
        savePath = ""
        extraDir = params["dir"]
        outputName = '/work/scratch/se55gyhe/tmps/tmp/' + (extraDir+"/"+name+"___"+str(time.time())+"___"+savePath)
        if not os.path.exists('/work/scratch/se55gyhe/tmps/tmp/'+extraDir): 
          os.makedirs('/work/scratch/se55gyhe/tmps/tmp/'+extraDir)
        self.my_outputName = outputName
    ...
    def computeF1Macro(self, sentences,average="micro"):
        correctLabels = [sentences[idx][self.labelKey] for idx in range(len(sentences))]
        predLabels = self.predictLabels(sentences)
        ...
        for sentenceId in range(len(correctLabels)):
            for tokenId in range(len(correctLabels[sentenceId])):
                y.append(correctLabels[sentenceId][tokenId])
                preds.append(predLabels[sentenceId][tokenId])

        if self.writeOutput:
            self.writeOutputToFile(sentences, predLabels, '%s' % ("myout"+self.flag))


        return f1_score(y, preds, average=average)

    def loadModel(self, modelPath):
        import h5py
        import json
        # from sequence_tagging.POS_tagging.keraslayers.ChainCRF import create_custom_objects
        #from sequence_tagging.POS_tagging.neuralnets.keraslayers.ChainCRF import create_custom_objects
        from .keraslayers.ChainCRF import create_custom_objects
        model = keras.models.load_model(modelPath, custom_objects={'MyLayer':create_custom_objects()})
